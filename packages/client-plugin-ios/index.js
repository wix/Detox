...
class Expect {
  ...
  createInvocation(expectation, ...params) {
    const definedParams = params.filter(x => x !== undefined);
    return {
      type: 'expectation',
...
class Element {
  ...
  createInvocation(action, targetElementMatcher, ...params) {
    params = params.map((param) => Number.isNaN(param) ? null : param);
...
class WaitFor {
  constructor(invocationManager, element) {
    this._invocationManager = invocationManager;
    this.element = new InternalElement(invocationManager, element.matcher, element.index);
