...
import { iosPool, testLog } from './_harness';
import type {
  CreatableDeviceType,
  DeviceTargetArgs,
  RawDeviceListing,
...
/** A pool over a fake SimulatorOps via the built-in iOS driver (spec 015), ephemeral gateway ports. */
function poolOf(simulatorOps: SimulatorOps, maxPool: number): DevicePool {
  return iosPool(simulatorOps, maxPool).pool;
}

/** Allocation ids are the core's (spec 015); the pool records whatever it is handed. */
let allocationSeq = 0;
const nextAllocationId = (): string => `alloc-${++allocationSeq}`;
...
describe('DevicePool.allocate', () => {
  ...
  it('refuses instantly when every matching device is busy, naming the holder', async () => {
    ...
    const pool = poolOf(simulatorOps, 4);

    const first = await pool.allocate({ allocationId: nextAllocationId(), query: {} });

    const err = await rejectionOf(pool.allocate({ allocationId: nextAllocationId(), query: {} }));
  ...
  it('never hands the same device to two concurrent callers', async () => {
    const pool = poolOf(fakeSimulatorOps({
        devices: [device('iPhone 17', 'udid-a'), device('iPhone 17', 'udid-b')],
      }).simulatorOps, 4);

    const [first, second] = await Promise.all([
      pool.allocate({ allocationId: nextAllocationId(), query: {} }),
      pool.allocate({ allocationId: nextAllocationId(), query: {} }),
    ]);
  ...
  it('caps every allocation, not only the ones that create a simulator', async () => {
    ...
    await pool.allocate({ allocationId: nextAllocationId(), query: {} });
    await expect(pool.allocate({ allocationId: nextAllocationId(), query: {} })).rejects.toThrow('All 1 device slots are taken (1 busy)');
  });
  ...
  it('keeps serving queued allocations after one of them fails', async () => {
    const pool = poolOf(fakeSimulatorOps().simulatorOps, 4);

    const failing = pool.allocate({ allocationId: nextAllocationId(), query: { byName: 'nothing' } });
    const alsoFailing = pool.allocate({ allocationId: nextAllocationId(), query: { byName: 'nothing' } });
  ...
  it('prefers an already-booted device — the cheapest allocation there is', async () => {
    ...
    expect((await pool.allocate({ allocationId: nextAllocationId(), query: {} })).udid).toBe('warm');
  });

  /**
   * @issue DTX-6234
   * The query is the contract: a client that asked for nothing in particular
   * has said that anything will do, and the pool may not overrule that — the
   * only thing that makes an answer wrong is failing to match what was
   * actually asked for.
   */
  it('answers an empty query with whatever it has, holding no opinion on shape', async () => {
    const pool = poolOf(fakeSimulatorOps({ devices: [device('iPad Pro 11-inch', 'ipad')] }).simulatorOps, 4);

    expect((await pool.allocate({ allocationId: nextAllocationId(), query: {} })).udid).toBe('ipad');
  });

  it('honours an explicit query', async () => {
    const pool = poolOf(fakeSimulatorOps({ devices: [device('iPad Pro 11-inch', 'ipad')] }).simulatorOps, 4);

    expect((await pool.allocate({ allocationId: nextAllocationId(), query: { byName: 'iPad Pro 11-inch' } })).udid).toBe('ipad');
  });
  ...
  it('consecutive refusals reuse the listing memo instead of re-listing', async () => {
    ...
    const pool = poolOf(simulatorOps, 1);
...
describe('devices in an unknown state (spec 005)', () => {
  it('refuses with a message that names the unknown state, not a phantom holder', async () => {
    ...
    const allocation = await pool.allocate({ allocationId: nextAllocationId(), query: {} });
  ...
  it('still says "busy" — and names the unknown ones — when both are in play', async () => {
    ...
    const first = await pool.allocate({ allocationId: nextAllocationId(), query: {} });
    const second = await pool.allocate({ allocationId: nextAllocationId(), query: {} });
  ...
  it('counts toward --max-pool, so a creation the pool would otherwise accept is refused', async () => {
    ...
    const err = await rejectionOf(pool.allocate({ allocationId: nextAllocationId(), query: { byType: 'iPhone 99' } }));
...
describe('DevicePool ownership (spec 002)', () => {
  ...
  it('a release presenting a stale allocation id frees nothing', async () => {
    ...
    expect(pool.release('udid-a', 'alloc-999')).toBe(false);
    expect(pool.busyCount).toBe(1);
    ...
    expect(pool.release('udid-a', first.allocationId)).toBe(false);
    expect(pool.busyCount).toBe(1);
  ...
  it('names every holder with hold age in the exhaustion payload', async () => {
    ...
    const pool = poolOf(simulatorOps, 2);
...
describe('DevicePool warm pool and eviction', () => {
  it('evicts the least recently used warm device when a claim needs its slot', async () => {
    ...
    const a = await pool.allocate({ allocationId: nextAllocationId(), query: {}, filter: only('udid-a') });
    pool.noteState('udid-a', 'Booted');
    pool.release('udid-a', a.allocationId);
    await new Promise((resolve) => setTimeout(resolve, 5));

    const b = await pool.allocate({ allocationId: nextAllocationId(), query: {}, filter: only('udid-b') });
    ...
    await pool.allocate({ allocationId: nextAllocationId(), query: {}, filter: only('udid-c') });
  ...
  it('a warm pick reuses its own slot instead of evicting anyone', async () => {
    ...
    for (const udid of ['udid-a', 'udid-b']) {
      const claim = await pool.allocate({ allocationId: nextAllocationId(), query: {}, filter: (d) => d.udid === udid });
    ...
    const again = await pool.allocate({ allocationId: nextAllocationId(), query: {}, filter: (d) => d.udid === 'udid-a' });
  ...
  /**
   * @issue DTX-6229
   * Eviction scope (spec 002): only devices this life handed out are
  ...
  it('never evicts a found-booted device and never counts it toward the cap', async () => {
    ...
    const claim = await pool.allocate({ allocationId: nextAllocationId(), query: {}, filter: (d) => d.udid === 'udid-c' });
...
describe('DevicePool create path (spec 002)', () => {
  ...
  it('does not overshoot the pool cap when creatable allocations race', async () => {
    ...
    const outcomes = await Promise.allSettled([
      pool.allocate({ allocationId: nextAllocationId(), query: { byType: 'iPhone 16 Pro' }, requestedType: 'ios.simulator' }),
      pool.allocate({ allocationId: nextAllocationId(), query: { byType: 'iPhone 16 Pro' }, requestedType: 'ios.simulator' }),
    ]);
  ...
  it('refuses terminally when nothing matches and nothing is creatable', async () => {
    ...
    await expect(
      pool.allocate({ allocationId: nextAllocationId(), query: { byType: 'Nokia 3310' }, requestedType: 'ios.simulator' }),
   ...
   * than guess: the original create error still surfaces, and nothing is
   * deleted.
   */
...
describe('DevicePool operational-state notes (spec 002)', () => {
  ...
  it('pushes a request-driven transition to the notifier and patches the tracked state', async () => {
    ...
    const claim = await pool.allocate({ allocationId: nextAllocationId(), query: {} });
    const pushes: string[] = [];
    ...
    const again = await pool.allocate({ allocationId: nextAllocationId(), query: {} });
...
describe('DevicePool eviction failure handling', () => {
  ...
  it('re-warms an evicted device when its shutdown attempt fails', async () => {
    ...
    try {
      ...
      const c = await pool.allocate({ allocationId: nextAllocationId(), query: {}, filter: only('udid-c') });
      expect(c.udid).toBe('udid-c');

      await vi.waitFor(() => expect(shutdownCalls).toEqual(['udid-a']));
      await vi.waitFor(() =>
        expect(errorSpy).toHaveBeenCalledWith(
          expect.stringContaining('eviction shutdown failed for udid-a'),
          expect.objectContaining({ udid: 'udid-a' }),
        ),
      );
    } finally {
      errorSpy.mockRestore();
...
describe('DevicePool.start (reconcile loop, spec 002)', () => {
   ...
   *
   * @issue DTX-2007
   * The push is sampled: the server polls the ground truth and pushes only
   * when it differs from what the client was last told — a tick that finds no
   * divergence pushes nothing, ever again for the same state.
   */
