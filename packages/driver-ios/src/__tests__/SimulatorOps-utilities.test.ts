...
import type { ExecOpts } from '@detox-remote/server';
...
describe('the quick utilities issue exactly one pinned command each', () => {
  it('uninstall names the bundle id and nothing else', async () => {
    await new SimulatorOps({ exec: execMock }).uninstall({ udid: UDID, bundleId: 'com.example.doomed' });
  ...
  it('openurl hands the URL over untouched — no resolution, no rewriting', async () => {
    await new SimulatorOps({ exec: execMock }).openUrl({ udid: UDID, url: 'http://127.0.0.1:8080/token?a=b#c' });
  ...
  it('location packs lat and lon into one comma-joined argv slot', async () => {
    await new SimulatorOps({ exec: execMock }).setLocation({ udid: UDID, lat: 32.0853, lon: 34.7818 });
  ...
  it('keychain reset is a first-party simctl verb, not a shell-out to applesimutils', async () => {
    await new SimulatorOps({ exec: execMock }).clearKeychain({ udid: UDID });
  ...
  it('status-bar override emits a flag ONLY for the fields the caller sent', async () => {
    await new SimulatorOps({ exec: execMock }).setStatusBar({ udid: UDID, overrides: { time: '12:34' } });
    expect(argvOf()).toEqual([
      '/usr/bin/xcrun',
      'simctl',
      'status_bar',
  ...
  it('treats 0 as a value, not as absence', async () => {
    await new SimulatorOps({ exec: execMock }).setStatusBar({
      udid: UDID,
  ...
  it('spawns no subprocess at all when there is nothing to override', async () => {
    await new SimulatorOps({ exec: execMock }).setStatusBar({ udid: UDID, overrides: {} });
    expect(execMock).not.toHaveBeenCalled();
  });

  it('status-bar reset clears every override', async () => {
    await new SimulatorOps({ exec: execMock }).resetStatusBar({ udid: UDID });
  ...
  it('enrollment sets and posts the exact notify key the readback uses', async () => {
    await new SimulatorOps({ exec: execMock }).setBiometricEnrollment({ udid: UDID, enabled: true });
    expect(argvOf()).toEqual([
      '/usr/bin/xcrun',
      'simctl',
      'spawn',
    ...
    execMock.mockClear();
    await new SimulatorOps({ exec: execMock }).setBiometricEnrollment({ udid: UDID, enabled: false });
  ...
  it('match/unmatch post the four one-shot keys, addressed by udid', async () => {
    const ops = new SimulatorOps({ exec: execMock });
    const cases = [
  ...
  it('erase is a bare `simctl erase <udid>`', async () => {
    await new SimulatorOps({ exec: execMock }).erase({ udid: UDID });
...
describe('a wedged erase is reported as an unknown device state', () => {
  ...
  it('turns a deadline kill into the typed device-unknown-state error', async () => {
    ...
    await expect(new SimulatorOps({ exec: execMock }).erase({ udid: UDID })).rejects.toMatchObject({
  ...
  it('lets an ordinary erase failure through untouched', async () => {
    ...
    const caught = await new SimulatorOps({ exec: execMock }).erase({ udid: UDID }).catch((err: unknown) => err);
    expect(caught).toBe(failure);
  });
});

describe('a wedged boot is bounded and reported as an unknown device state', () => {
  /** `_isBooted` lists first; the boot verbs are the calls after it. */
  function bootExecs(): ExecOpts[] {
    return execMock.mock.calls.map((call) => call[0] as ExecOpts).filter((o) => o.args[1] !== 'list' && o.file === '/usr/bin/xcrun');
  }

  it('gives `simctl boot` and `simctl bootstatus` each a deadline', async () => {
    execMock.mockImplementation((opts: ExecOpts) =>
      Promise.resolve({ stdout: opts.file === 'applesimutils' ? '[]' : '', stderr: '' }),
    );
    await new SimulatorOps({ exec: execMock }).boot({ udid: UDID });
    const [boot, bootstatus] = bootExecs();
    expect(boot.args.slice(0, 2)).toEqual(['simctl', 'boot']);
    expect(boot.timeout).toBeGreaterThan(0);
    expect(bootstatus.args.slice(0, 2)).toEqual(['simctl', 'bootstatus']);
    expect(bootstatus.timeout).toBeGreaterThan(0);
  });

  it('turns a deadline kill of either verb into the typed device-unknown-state error', async () => {
    for (const verb of ['boot', 'bootstatus']) {
      execMock.mockReset();
      execMock.mockImplementation((opts: ExecOpts) =>
        opts.args[1] === verb
          ? Promise.reject(Object.assign(new Error(`simctl ${verb} killed`), { killed: true }))
          : Promise.resolve({ stdout: opts.file === 'applesimutils' ? '[]' : '', stderr: '' }),
      );
      await expect(new SimulatorOps({ exec: execMock }).boot({ udid: UDID }), verb).rejects.toMatchObject({
        name: 'DeviceUnknownStateError',
        code: DetoxErrorCode.DETOX_DEVICE_UNKNOWN_STATE,
        details: { udid: UDID, command: `simctl ${verb}` },
      });
    }
  });

  it('lets an ordinary boot failure through untouched', async () => {
    const failure = new Error('Unable to boot device in current state: Booted');
    execMock.mockImplementation((opts: ExecOpts) =>
      opts.args[1] === 'boot'
        ? Promise.reject(failure)
        : Promise.resolve({ stdout: opts.file === 'applesimutils' ? '[]' : '', stderr: '' }),
    );
    const caught = await new SimulatorOps({ exec: execMock }).boot({ udid: UDID }).catch((err: unknown) => err);
    expect(caught).toBe(failure);
  });
});
...
describe('argv-shaped values are refused, typed', () => {
  ...
  it.each([
    ...
    await expect(call(new SimulatorOps({ exec: execMock }))).rejects.toMatchObject({
 ...
 * rollback retained for a minute can reach a device that has since been
 * shut down, evicted, or wiped; there "your app is not
...
describe('terminate tolerates a device that is down — only when asked to', () => {
  ...
  it('rethrows for an ordinary caller', async () => {
    ...
    await expect(
      new SimulatorOps({ exec: execMock }).terminate({ udid: UDID, bundleId: 'com.example.app' }),
  ...
  it('swallows it for a compensation', async () => {
    execMock.mockRejectedValueOnce(downDevice);
    await expect(
      new SimulatorOps({ exec: execMock }).terminate({
        udid: UDID,
