...
describe('setPermissions — the dispatch table, argv-exact', () => {
  ...
  it('medialibrary spells itself media-library on the simctl line (v20 parity)', async () => {
    await new SimulatorOps({ exec: execMock }).setPermissions({
      udid: UDID,
      bundleId: BUNDLE,
  ...
  it("an unknown service is a typed refusal, and nothing runs — v20's fall-through hole", async () => {
    await expect(
      new SimulatorOps({ exec: execMock }).setPermissions({
        udid: UDID,
        bundleId: BUNDLE,
...
describe('launch argv composition (spec 006)', () => {
  ...
  it('orders user args first, payload argv next, the frozen detox pair LAST', async () => {
    await new SimulatorOps({ exec: execMock }).launch({
      udid: UDID,
      bundleId: BUNDLE,
...
describe('resume — a foreground is not a launch transaction', () => {
  ...
  it('runs a bare simctl launch: no argv, no detox pair, no injection env', async () => {
    await new SimulatorOps({ exec: execMock }).resume({ udid: UDID, bundleId: BUNDLE });
...
describe('sendToHome keeps only wedge detectors (spec 006)', () => {
   ...
   * either hands its message to CoreSimulator in seconds or is wedged, and an
   * unbounded child under the reclaim barrier would hold `release` hostage.
   * The launch leg ran with no timeout until spec 006.
   */
  it('bounds both children — the launch leg ran with no timeout until spec 006', async () => {
    await new SimulatorOps({ exec: execMock }).sendToHome({ udid: UDID });
    expect(execMock).toHaveBeenCalledTimes(2);
...
describe('the app-output capture (spec 013)', () => {
  it('puts --stdout=/--stderr= before the device, so they are simctl options, never the app\'s argv', async () => {
    await new SimulatorOps({ exec: execMock }).launch({
      udid: UDID,
      bundleId: BUNDLE,
      launchArgs: { a: 1 },
      output: { stdout: '/dev/x/data/tmp/o.out', stderr: '/dev/x/data/tmp/o.err' },
    });
    expect(argvOf(0)).toEqual([
      '/usr/bin/xcrun',
      'simctl',
      'launch',
      '--stdout=/dev/x/data/tmp/o.out',
      '--stderr=/dev/x/data/tmp/o.err',
      UDID,
      BUNDLE,
      '-a',
      '1',
