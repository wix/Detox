import type { ExecOpts, ExecResult } from '@detox-remote/server';
...
/** The OS a simulator runs, as `applesimutils --list` reports it. */
...
/** One simulator as the listing reports it — `simctl`'s own vocabulary (the pool's, never the wire's). */
...
export interface LaunchAppArgs {
  ...
  /**
   * Where `simctl launch` writes the app's own stdout/stderr (spec 013):
   * `--stdout=<file> --stderr=<file>`, both inside the simulator's data
   * directory (`app-output.ts` — anywhere else captures nothing). Absent,
   * the streams go where simctl sends them by default.
   */
 ...
 * runtime warming up); shrinking it on the assumption that launches are
 * fast would break cold boots. `retries: 0` is still load-bearing: a re-run of
...
/**
 * Ceiling on `simctl boot`, per attempt. The command only asks CoreSimulator
 * to start the device and returns in seconds; the actual boot is waited out
 * by `bootstatus`. Anything past this is a wedged CoreSimulator.
 */
const BOOT_TIMEOUT_MS = 60_000;

/**
 * Ceiling on `simctl bootstatus`, the one call that waits for a boot to
 * complete. A cold boot of a fresh runtime on a loaded machine is the
 * measured slow case, well under a minute; shrinking this on the assumption
 * that boots are fast would break cold boots. A kill leaves the device
 * unknown-state, reported by `unknownStateIfKilled`.
 */
...
/** What `SimulatorOps` is built from: the server's child-process runner, off the toolkit (spec 013 spawn tracing). */
export interface SimulatorOpsDeps {
  exec: (opts: ExecOpts) => Promise<ExecResult>;
}

export class SimulatorOps {
  private readonly _exec: (opts: ExecOpts) => Promise<ExecResult>;

  constructor({ exec }: SimulatorOpsDeps) {
    this._exec = exec;
  }
  ...
  async boot({ udid, bootArgs = [], headless = true, signal, onBootStart }: BootArgs): Promise<boolean> {
    ...
    try {
      try {
        await this._execSimctl({ args: ['boot', udid, ...bootArgs], signal, retries: 10, timeout: BOOT_TIMEOUT_MS });
      } catch (err) {
        throw unknownStateIfKilled(err, { udid, command: 'simctl boot', timeoutMs: BOOT_TIMEOUT_MS });
      }
      try {
        await this._execSimctl({ args: ['bootstatus', udid], signal, timeout: BOOTSTATUS_TIMEOUT_MS });
      } catch (err) {
        throw unknownStateIfKilled(err, { udid, command: 'simctl bootstatus', timeoutMs: BOOTSTATUS_TIMEOUT_MS });
      }
    } catch (err) {
  ...
  async launch({
    ...
    // reproduced here.
    // `--stdout=`/`--stderr=` are simctl's own options and precede the device
    // (everything after the bundle id belongs to the app).
    const capture = output ? [`--stdout=${output.stdout}`, `--stderr=${output.stderr}`] : [];
    const result = await this._execSimctl({
      args: ['launch', ...capture, udid, bundleId, ...merged.flat()],
      signal,
      retries: 0,
   ...
   *
   * Blast radius: every applesimutils-backed invocation carries
   * `--restartSB` — SpringBoard
   ...
   * `installApp` draws the opposite line and does fetch an http(s) archive).
   * Ported from Detox 20
   ...
   * around it lives on the lease (`IosSimulatorLease.resetContentAndSettings`),
  ...
  private _execSimctl({ args, signal, retries = 1, timeout, env }: ExecSimctlArgs): Promise<ExecResult> {
    return this._exec({ file: '/usr/bin/xcrun', args: ['simctl', ...args], signal, retries, timeout, env });
  }
