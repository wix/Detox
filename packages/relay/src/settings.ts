/** `detox relay`'s settings — mirrors `SERVER_SETTINGS`. `--nodes` is not
 *  here: it names a file, not a value, and its required-ness depends on
 *  the caller's own inline-roster flag (`resolveRelayCli`'s business). */
import { z } from 'zod';
import type { SettingDescriptor } from '@detox-remote/core';
import { isLogLevel, parseDuration } from '@detox-remote/server';
...
export const RELAY_SETTINGS = [
  {
    ...
    schema: zNonEmpty,
    flag: '--host',
    env: 'DETOX_RELAY_HOST',
    help: 'Interface to bind (default: 127.0.0.1)',
  },
  {
    key: 'port',
    schema: z.coerce.number().int().min(0).max(65_535, 'expected 0-65535'),
    flag: '--port',
    env: 'DETOX_RELAY_PORT',
    help: 'Port to listen on (default: 0 = pick a free one)',
  },
  {
    key: 'token',
    schema: zNonEmpty,
  ...
  },
  {
    ...
    env: 'DETOX_RELAY_KEEPALIVE_WINDOW',
    help:
      'Seconds a client may stay unresponsive before its session ends (default: 120, ' +
      'max: 604800 = 7 days). 0 turns liveness polls off entirely.',
  },
  {
    key: 'blobBudget',
    schema: zPositiveBytes,
    flag: '--blob-budget',
    env: 'DETOX_RELAY_BLOB_BUDGET',
    help: "Byte budget of the relay's own build-cache mirror (default: 8 GiB)",
  },
  {
    key: 'blobRoot',
    schema: zNonEmpty,
    env: 'DETOX_RELAY_BLOB_ROOT',
    internal: true,
  },
  {
    key: 'logLevel',
