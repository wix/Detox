...
import type { DetoxOperationRef, DetoxProgressEvent } from 'detox/internals';
 ...
 * fail, and it must fail as a rejection (init returns a promise, so `.catch()`
 * has to work).
 */
test('init rejects when the server is unreachable', async (t) => {
  await assert.rejects(
    init({ server: 'ws://127.0.0.1:1', signal: t.signal }),
...
test('aborting the session aborts calls in flight', async (t) => {
  ...
  const detox = await init({ server: server.address, signal: session.signal });
