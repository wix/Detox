...
test('exhaustion is typed, names the holder across connections, and is honest', async (t) => {
  ...
  await using owner = await init({ server: server.address, signal: t.signal });
  await using bystander = await init({ server: server.address, signal: t.signal });
...
test('turned away is not nobody home', async (t) => {
  ...
  const unauthorized = assertDetoxError(
    await rejectionOf(init({ server: server.address, signal: t.signal }), 'wrong token'),
    'unauthorized',
  );
  ...
  const unreachable = assertDetoxError(
    await rejectionOf(init({ server: { url: deadUrl }, signal: t.signal }), 'dead address'),
    'unreachable',
  );
...
test('a dying server speaks its code', async (t) => {
  ...
  const detox = await init({ server: server.address, signal: t.signal });
  try {
