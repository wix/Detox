...
module.exports = function ({ typeCheckInterfaces, renameTypesMap, supportedTypes, classValue, supportedContentSanitizersMap }) {
  ...
  function createClass(json) {
    return t.classDeclaration(
      ...
      null,
      t.classBody(json.methods.filter(filterMethodsWithUnsupportedParams).map(createMethod.bind(null, json))),
      []
    );
  }

  function filterMethodsWithUnsupportedParams(method) {
    return method.args.reduce((carry, methodArg) => carry && supportedTypes.includes(methodArg.type), true);
  }
