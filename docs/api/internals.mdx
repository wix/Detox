...
On one hand, Detox CLI can be totally agnostic about a test runner under the hood, but that would deprive us of some conveniences.
...
Even we run Jest directly, without Detox CLI, there's still a one-to-many relationship between its processes:
...
There's one more implicit thing that happens at the very beginning of [`init([options])`][init] method, and this is the config resolution. It is also available as a separate method, [`resolveConfig([options])`][resolveConfig] based on the following considerations.
...
The global cleanup phase should happen when the main or child process is about to exit. Under the hood, it:
...
Not documented on purpose. Provides the direct access to the object which holds the device driver, websocket client, matchers, expectations, etc.
...
[typings]: https://github.com/wix/Detox/blob/next/detox/internals.d.ts
