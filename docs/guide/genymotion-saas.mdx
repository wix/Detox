...
### `behavior.shutdownDevice` is ignored

Detox CLI has [`-u, --cleanup` argument], and the behavior config has the respective [`shutdownDevice`
property], but **none of them has effect** on Genymotion SaaS devices. Detox _always shuts them down_ in the end
of the test session, unless you abruptly terminate its execution (see the previous caveat).
...
[`-u, --cleanup` argument]: ../api/detox-cli.md#test
