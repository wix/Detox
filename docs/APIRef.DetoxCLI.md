...
| -a, --artifacts-location \<path\>             | Artifacts (logs, screenshots, etc) root directory. For more details, please check the [Enabling artifacts](APIRef.Artifacts.md#artifacts) |
| --record-logs [failing\|all\|none]            | Save logs during each test to artifacts directory. Pass "failing" to save logs of failing tests only. |
| --take-screenshots [failing\|all\|none]       | Save screenshots before and after each test to artifacts directory. Pass "failing" to save screenshots of failing tests only. |
| --record-videos [failing\|all\|none]          | Save screen recordings of each test to artifacts directory. Pass "failing" to save recordings of failing tests only. 
