...
Detox provides a REPL (Read-Eval-Print Loop) mode that allows you to interactively debug your app during test execution. This feature is built on top of Node.js' [`node:repl`][1] module and is particularly useful when you need to explore the app's state, try different commands, or debug test failures.
...
- Use the `.ai` command to execute natural language commands (e.g., `.ai Tap on login button`). Note that you must have [Detox Pilot][2] configured before you start.
- Access all Detox globals like `device`, `element`, `by`, etc.

<details>

  <summary>💡 <b>Adding scope to REPL context</b></summary>
