...
This workflow is mostly useful for investigating weird crashes or when contributing to Detox itself.
...
[Configuration > Artifacts](../config/artifacts.md) document for the reference.
...
Don't use multiple workers (`-w, --maxWorkers`) if you set `session.sessionId` to a constant value.
