...
### `behavior.optimizeReinstall` \[boolean]

Default: `false`.

When _true_, Detox will use [`device.resetAppState()`](../api/device.md#deviceresetappstatebundleids) instead of uninstalling and reinstalling the app during [`device.launchApp({delete: true})`](../api/device.md#devicelaunchappparams).

```js
// In your detox.config.js
module.exports = {
  behavior: {
