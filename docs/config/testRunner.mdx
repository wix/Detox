...
When true, tells `detox test` to cancel next retrying if it gets at least one report about a [permanent test suite failure](../api/internals.md#reporting-test-results).
