...
| `useSeparateAdbServers` | _Optional. Boolean. Supported by `android.emulator` only._ <br/> `false` by default. When set to `true`, Detox-managed emulators use dedicated ADB servers on separate ports instead of sharing the default ADB server on port `5037`. This is an opt-in stabilization mode for parallel Android-emulator runs. Detox keeps a lockfile-backed registry of reserved custom ADB ports, preserves reservations while emulators are booting, and restores the matching `ANDROID_ADB_SERVER_PORT` in the worker process for the currently allocated emulator. |
