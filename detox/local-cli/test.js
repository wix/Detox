...
const GenyCloudDeviceRegistry = require('../src/devices/drivers/android/genycloud/GenyCloudDeviceRegistry');
...
async function resetLockFile({ platform }) {
  ...
  if (platform === 'android') {
    ...
    await GenyCloudDeviceRegistry.forGlobalCleanup().reset();
  }
}
