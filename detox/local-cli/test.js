...
module.exports.builder = {
  ...
  'debug-synchronization': {
    ...
    group: 'Debugging',
    describe: 'When an action/expectation takes a significant amount of time use this option to print device synchronization status.'
  ...
  'take-screenshots': {
    ...
    choices: ['failing', 'all', 'none'],
    default: 'none',
    describe: '[EXPERIMENTAL] Save screenshots before and after each test to artifacts directory. Pass "failing" to save screenshots of failing tests only.'
  },
  'record-videos': {
    alias: 'recordVideos',
    group: 'Debugging',
    choices: ['failing', 'all', 'none'],
    default: 'none',
    describe: '[EXPERIMENTAL] Save screen recordings of each test to artifacts directory. Pass "failing" to save recordings of failing tests only.'
  },
  file: {
    alias: 'f',
    group: 'Configuration',
  ...
  workers: {
    alias: 'w',
    group: 'Execution',
    describe: '[iOS Only] Specifies number of workers the test runner should spawn, requires a test runner with parallel execution support (Detox CLI currently supports Jest)',
    default: 1,
    number: true
  },
  'device-name': {
...
module.exports.handler = function main(program) {
  ...
  function collectExtraArgs(argv) {
    const aliasKeys = Object
      .values(module.exports.builder)
      .reduce((carry, item) => 
        [].concat(carry, typeof item.alias === "string" ? [item.alias] : item.alias), []
      );
    const knownKeys = [].concat(Object.keys(module.exports.builder), aliasKeys);
    const clone = Object.assign({}, argv);
    knownKeys.forEach(key => delete clone[key]);

    return Object.entries(([key, value]) => `--${key} ${value}`).join(' ');
  }
