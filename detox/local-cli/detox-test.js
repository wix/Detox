...
program
  ...
  .option('-s, --specs [relativePath]', `Root of test folder`)
  ...
  .option('-f, --file [path]', 'Specify test file to run')
  .parse(process.argv);
...
if (program.configuration) {
  ...
  Use --configuration to choose one of the following: ${_.keys(
...
const platform = config.configurations[program.configuration].type.split(
  '.',
)[0];
...
function run() {
  switch (runner) {
    ...
    default:
      throw new Error(
        `${runner} is not supported in detox cli tools. You can still run your tests with the runner's own cli tool`,
      );
  }
}
...
function runMocha() {
  ...
  const artifactsLocation = program.artifactsLocation
    ? `--artifacts-location ${program.artifactsLocation}`
    : '';
  const headless = program.headless ? `--headless` : '';
  const opts = getMochaOpts();
  const debugSynchronization = program.debugSynchronization
