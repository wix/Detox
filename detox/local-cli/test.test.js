...
describe('test', () => {
  ...
  describe('mocha', () => {
    it('runs successfully', async () => {
      ...
      expect(mockExec).toHaveBeenCalledWith(
        expect.stringContaining(`${normalize('node_modules/.bin/mocha')} --opts e2e/mocha.opts --configuration only --grep :ios: --invert --record-logs none --take-screenshots none --record-videos none --record-performance none --artifacts-location "${normalize('artifacts/only.')}`),
        expect.anything()
      );
  ...
  it('sets default value for debugSynchronization', async () => {
    ...
    expect(mockExec).toHaveBeenCalledWith(
      expect.stringContaining(
        `${normalize('node_modules/.bin/mocha')} --opts e2e/mocha.opts --configuration only --debug-synchronization 3000 --grep :ios: --invert --record-logs none --take-screenshots none --record-videos none --record-performance none --artifacts-location "${normalize('artifacts/only.')}`
      ),
      expect.anything()
    );
  });
