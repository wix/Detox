...
describe("Animations", () => {
  ...
  async function _startTest(driver, options = {}) {
    let driverControlSegment = element(
      by
        .text(driver)
        .withAncestor(by.id("UniqueId_AnimationsScreen_useNativeDriver"))
    );
    ...
    if (options.loops !== undefined) {
      ...
      await element(
        by.id("UniqueId_AnimationsScreen_numberOfIterations")
      ).replaceText(String(options.loops));
    }

    if (options.duration !== undefined) {
      await element(by.id("UniqueId_AnimationsScreen_duration")).replaceText(
        String(options.duration)
      );
    }

    if (options.delay !== undefined) {
      await element(by.id("UniqueId_AnimationsScreen_delay")).replaceText(
  ...
  _.forEach(["JS", "Native"], driver => {
    it(`should find element (driver: ${driver})`, async () => {
      ...
      await expect(
        element(by.id("UniqueId_AnimationsScreen_afterAnimationText"))
      ).toBeVisible();
    });
    ...
    it.skip(
      `should not wait for infinite animations (driver: ${driver})`,
      async () => {
