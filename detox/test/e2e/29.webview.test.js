...
describe('WebView', () => {
  ...
  describe('Actions', () => {
    ...
    it('should run script on element', async () => {
      const textInput = web.element(by.web.id('textInput'));

      const result = await textInput.runScript('(el) => el.getAttribute("aria-label")');
      jestExpect(result).toBe('textInputLabel-detox-autogenerated-0');
    });
