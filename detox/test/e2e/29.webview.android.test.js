...
describe(':android: WebView', () => {
  ...
  describe('Expectations',() => {
    it('expect element to exists', async () => {
      await expect(webview.element(by.web.id('testingPar'))).toExist();
    });

    it('expect element to NOT exists', async () => {
      await expect(webview.element(by.web.id('not_found'))).not.toExist();
    });

    it('expect element to have text', async () => {
      await expect(webview.element(by.web.id('testingPar'))).toHaveText('Message');
    });

    it('expect element to NOT have text', async () => {
      await expect(webview.element(by.web.id('testingPar'))).not.toHaveText(MOCK_TEXT);
    });
  });

  describe('Element Matchers',() => {
    it('expect to find element by id', async () => {
      await expect(webview.element(by.web.id('testingh1'))).toExist();
    });

    it('expect to find element by class name', async () => {
      await expect(webview.element(by.web.className('a'))).toExist();
    });

    it('expect to find element by css selector', async () => {
      await expect(webview.element(by.web.cssSelector('#cssSelector'))).toExist();
    });

    it('expect to find element by name', async () => {
      await expect(webview.element(by.web.name('sec_input'))).toExist();
    });

    it('expect to find element by xpath', async () => {
      await expect(webview.element(by.web.xpath('//*[@id="testingh1-1"]'))).toExist();
    });

    it('expect to find element by href', async () => {
      await expect(webview.element(by.web.href('disney.com'))).toExist();
    });

    it('expect to find element by hrefContains', async () => {
      await expect(webview.element(by.web.hrefContains('disney'))).toExist();
    });

    it('expect to find element by tag name', async () => {
      await expect(webview.element(by.web.tag('mark'))).toExist();
    });
  });
  ...
  describe('ContentEditable', () => {
    ...
    it('should replace text by selecting all text', async () => {
        const editable = await webview.element(by.web.className('public-DraftEditor-content'));
  ...
  it('should set input and change text', async () => {
    ...
    const para = webview.element(by.web.id('testingPar'));
    await expect(para).toHaveText('Message');

    const textInput = await webview.element(by.web.id('textInput'));
    await textInput.scrollToView();
    await textInput.tap();
    await textInput.typeText(MOCK_TEXT);

    await webview.element(by.web.id('changeTextBtn')).tap();
    // Verify text updated
    await expect(para).toHaveText(MOCK_TEXT);
  });

  it('should header get text and verify its value', async () => {
    const text = await webview.element(by.web.id('testingh1')).getText();
    ...
    await expect(webview.element(by.web.id('testingPar'))).toHaveText(text);
  ...
  it('should replace text', async () => {
    ...
    await webview.element(by.web.id('changeTextBtn')).tap();
    await expect(webview.element(by.web.id('testingPar'))).toHaveText('first text');

    await textInput.replaceText(MOCK_TEXT);
    await webview.element(by.web.id('changeTextBtn')).tap();

    // Verify param value is the latest changed text
    await expect(webview.element(by.web.id('testingPar'))).toHaveText(MOCK_TEXT);
  });

  it('should locate single web-view', async () => {
    webview = web.element;
    await expect(webview.element(by.web.id('testingDiv'))).toHaveText('Testing Heading');
  });
});
