...
describe("Network Synchronization", () => {
  ...
  it("Sync with short network requests - 100ms", async () => {
    ...
    await expect(
      element(by.label("Short Network Request Working!!!"))
    ).toBeVisible();
  });
  ...
  it("Sync with long network requests - 3000ms", async () => {
    ...
    await expect(
      element(by.label("Long Network Request Working!!!"))
    ).toBeVisible();
  });

  it("disableSynchronization() should disable sync", async () => {
    await device.disableSynchronization();
    await waitFor(element(by.id("LongNetworkRequest")))
      .toBeVisible()
      .withTimeout(4000);
    await element(by.id("LongNetworkRequest")).tap();
    await expect(
    ...
    await waitFor(element(by.label("Long Network Request Working!!!")))
      .toBeVisible()
      .withTimeout(4000);
    await expect(
      element(by.label("Long Network Request Working!!!"))
