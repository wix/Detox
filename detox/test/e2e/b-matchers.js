describe("Matchers", () => {
  ...
  it("should match elements by type (native class)", async () => {
    const byType =
      device.getPlatform() === "ios"
        ...
        : by.type("android.widget.ImageView");
  ...
  it("should match elements with ancenstor (parent)", async () => {
    ...
    await expect(
      element(by.id("Son883").withAncestor(by.id("Grandson883")))
    ).toNotExist();
    await expect(
      element(by.id("Grandson883").withAncestor(by.id("Father883")))
    ).toExist();
    await expect(
      element(by.id("Father883").withAncestor(by.id("Grandson883")))
    ).toNotExist();
    await expect(
      element(by.id("Grandson883").withAncestor(by.id("Grandfather883")))
    ).toExist();
    await expect(
      element(by.id("Grandfather883").withAncestor(by.id("Grandson883")))
    ).toNotExist();
  });
  ...
  it("should match elements with descendant (child)", async () => {
    ...
    await expect(
      element(by.id("Grandson883").withDescendant(by.id("Son883")))
    ).toNotExist();
    await expect(
      element(by.id("Father883").withDescendant(by.id("Grandson883")))
    ).toExist();
    await expect(
      element(by.id("Grandson883").withDescendant(by.id("Father883")))
    ).toNotExist();
    await expect(
      element(by.id("Grandfather883").withDescendant(by.id("Grandson883")))
    ).toExist();
    await expect(
      element(by.id("Grandson883").withDescendant(by.id("Grandfather883")))
    ).toNotExist();
  });
  ...
  it("should match elements by using two matchers together with and", async () => {
    ...
    await expect(
      element(by.id("UniqueId345").and(by.label("RandomJunk")))
    ).toNotExist();
  });
  ...
  it.skip(
    ...
    async () => {
      await expect(element(by.label("Product")).atIndex(2)).toHaveId(
        "ProductId002"
      );
    }
  );
