describe("Assertions", () => {
  ...
  it("should assert an element has text", async () => {
    await expect(element(by.id("UniqueId204"))).toHaveText(
      "I contain some text"
    );
  });
  ...
  it("should assert an element has (accessibility) label", async () => {
    await expect(element(by.id("UniqueId204"))).toHaveLabel(
      "I contain some text"
    );
  });

  it("should assert an element has (accessibility) id", async () => {
    await expect(element(by.label("I contain some text"))).toHaveId(
      "UniqueId204"
    );
  });
