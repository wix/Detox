describe("WaitFor", () => {
  ...
  it("should wait until an element is created and exists in layout", async () => {
    ...
    await waitFor(element(by.id("createdAndVisibleText")))
      .toExist()
      .withTimeout(2000);
  ...
  it("should wait until an invisible element becomes visible", async () => {
    await expect(
      element(by.id("invisibleBecomingVisibleText"))
    ).toBeNotVisible();
    await waitFor(element(by.id("invisibleBecomingVisibleText")))
      .toBeVisible()
      .withTimeout(2000);
  ...
  it("should wait until an element is removed", async () => {
    ...
    await waitFor(element(by.id("deletedFromHierarchyText")))
      .toBeNotVisible()
      .withTimeout(2000);
  ...
  it("should find element by scrolling until it is visible", async () => {
    ...
    await waitFor(element(by.label("Text5")))
      .toBeVisible()
