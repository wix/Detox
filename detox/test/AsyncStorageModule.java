...
public final class AsyncStorageModule
  ...
  private class SerialExecutor implements Executor {
    ...
    public synchronized void execute(final Runnable r) {
      Log.d("ASYNC_STORAGE", "Enqueuing task, pendingTasks#="+mTasks.size());
      mTasks.offer(new Runnable() {
        public void run() {
          try {
            Log.d("ASYNC_STORAGE", "Executing a task!");
            r.run();
          } finally {
            scheduleNext();
          }
    ...
    synchronized void scheduleNext() {
      if ((mActive = mTasks.poll()) != null) {
        Log.d("ASYNC_STORAGE", "Scheduling next task, pendingTasks#="+mTasks.size());
        executor.execute(mActive);
      } else {
        Log.d("ASYNC_STORAGE", "NOT scheduling next task, pendingTasks#="+mTasks.size());
      }
    }
  }
  ...
  public void multiGet(final ReadableArray keys, final Callback callback) {
    ...
    new GuardedAsyncTask<Void, Void>(getReactApplicationContext()) {
      ...
      protected void doInBackgroundGuarded(Void... params) {
        Log.d("ASYNC_STORAGE", "multiGet() task");
        if (!ensureDatabase()) {
        ...
        for (int keyStart = 0; keyStart < keys.size(); keyStart += MAX_SQL_KEYS) {
          ...
          try {
            ...
            Log.d("ASYNC_STORAGE", "multiGet() task => error");
        ...
        Log.d("ASYNC_STORAGE", "multiGet() task => success");
  ...
  public void multiSet(final ReadableArray keyValueArray, final Callback callback) {
    ...
    new GuardedAsyncTask<Void, Void>(getReactApplicationContext()) {
      @Override
      protected void doInBackgroundGuarded(Void... params) {
        Log.d("ASYNC_STORAGE", "multiSet() task");
        ...
        if (error != null) {
          Log.d("ASYNC_STORAGE", "multiSet() task => error");
          callback.invoke(error);
        } else {
          Log.d("ASYNC_STORAGE", "multiSet() task => success");
          callback.invoke();
        }
      }
    }.executeOnExecutor(executor);
  ...
  public void getAllKeys(final Callback callback) {
    new GuardedAsyncTask<Void, Void>(getReactApplicationContext()) {
      @Override
      protected void doInBackgroundGuarded(Void... params) {
        Log.d("ASYNC_STORAGE", "getAllKeys()");
        if (!ensureDatabase()) {
          callback.invoke(AsyncStorageErrorUtil.getDBError(null), null);
          return;
        }
        ...
        try {
          ...
          FLog.w(ReactConstants.TAG, e.getMessage(), e);
          Log.d("ASYNC_STORAGE", "getAllKeys() => error");
          callback.invoke(AsyncStorageErrorUtil.getError(null, e.getMessage()), null);
          return;
        } finally {
          cursor.close();
        }
        Log.d("ASYNC_STORAGE", "getAllKeys() => success");
        callback.invoke(null, data);
      }
    }.executeOnExecutor(executor);
  }
