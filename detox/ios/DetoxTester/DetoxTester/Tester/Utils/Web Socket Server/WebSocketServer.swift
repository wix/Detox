...
class WebSocketServer {
  ...
  private init(withPort port: UInt16, delegate: WebSocketServerDelegateProtocol) throws {
    ...
    guard let port = NWEndpoint.Port(rawValue: port) else {
      wsServerLog("unable to establish web-socket server on port \(port)", type: .error)
      throw Error.invalidPort
    }

    listener = try NWListener(using: parameters, on: port)

    wsServerLog("web-socket server established on port \(port)")
  ...
  func startServer() {
    listener.newConnectionHandler = { newConnection in
      if let previousClient = self.client {
        if previousClient.endpoint == newConnection.endpoint {
          wsServerLog("server is already connected to this client", type: .error)
          return
        }

        wsServerLog(
          "server is already connected to a client, cancelling previous connection",
          type: .error
        )
      ...
      wsServerLog("new connection with tester server")
      self.client = newConnection

      func receive() {
        newConnection.receiveMessage { (data, context, isComplete, error) in
          if let data = data {
            wsServerLog("received a new message from client")
      ...
      newConnection.stateUpdateHandler = { state in
        switch state {
          case .ready:
            wsServerLog("[new connection update] connection with client is ready")
          ...
          case .setup, .cancelled, .preparing:
            wsServerLog("[new connection update] connection with client state has changed to `\(state)`")

          @unknown default:
            wsServerLog("[new connection update] connection with client is unknown")
        }
      }
    ...
    listener.stateUpdateHandler = { state in
      switch state {
        case .ready:
          wsServerLog("[listener state changed] tester server is ready")
        ...
        case .setup, .cancelled:
          wsServerLog("[listener state changed] server state has changed to `\(state)`")

        @unknown default:
          wsServerLog("[listener state changed] server state is unknown")
      }
    }
  ...
  func sendMessage(data: Data) throws {
    guard let client = client else {
      wsServerLog("no client is available, can't send message", type: .error)
  ...
  private func sendMessage(toClient client: NWConnection, data: Data) throws {
    ...
    client.send(
      ...
      completion: .contentProcessed({ error in
        if let error = error {
          wsServerLog("client error when sending message: \(error.localizedDescription)", type: .error)
        }
      })
    )
  }
  ...
  func closeClientConnection() {
    guard let previousClient = self.client else {
      wsServerLog("server called to close connection while not connected to any client", type: .error)
      fatalError("server called to close connection while not connected to any client")
    }

    wsServerLog("closing client connection", type: .debug)
    previousClient.cancel()
    delegate.didCloseConnection()
    wsServerLog("client connection did close", type: .debug)
  }
}
