...
class ActionDelegate: ActionDelegateProtocol {
  ...
  func getAttributes(from elements: [AnyHashable]) throws -> AnyCodable {
    ...
    uiLog("fetching white-box attributes from elements (if enabled)", type: .debug)
    let whiteBoxAttributes = whiteBoxMessageHandler(
      .requestAttributes(ofElements: mappedElements)
    ) ?? .elementsAttributes(Array.init(repeating: .init(value: [:]), count: mappedElements.count))
