...
extension XCUIElementQuery {
  ...
  private func createJSElementsMatcher(pattern: WebElementPattern) throws -> String {
    switch pattern {
      case .id(let id):
        return "{ return Array.from(document.getElementsByClassName('\(id)')); }"

      case .className(let className):
        return "{ return Array.from(document.getElementsByClassName('\(className)')); }"

      case .cssSelector(let cssSelector):
        return "{ return Array.from(document.querySelectorAll('\(cssSelector)')); }"

      case .name(let name):
        return "{ return Array.from(document.getElementsByName('\(name)')); }"
      ...
      case .href(let href):
        return "{ return Array.from(document.querySelectorAll('[href=\"\(href)\"]')); }"

      case .hrefContains(let substring):
        return "{ return Array.from(document.querySelectorAll('[href*=\"\(substring)\"]')); }"

      case .tag(let tag):
        return "{ return Array.from(document.getElementsByTagName('\(tag)')); }"
