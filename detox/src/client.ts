/**
 * `detox/client` — the public client dialect for Detox v21.
 *
 ...
 * spec-001 prototype client into public naming: `connect` + handles instead
 * of `DeviceAllocator`.
 *
 *   import { connect } from 'detox/client';
 *   await using detox = await connect({ server: 'ws://localhost:3456', signal });
 ...
 * `connect` is the exception: no handle exists yet to hang a subscription on, so
 * connecting narrates through `options.onProgress` only.
 */
import { connectSession } from './client/session';
import { DetoxError, DetoxErrorCode } from './client/errors';
 ...
 *
 * A call inherits its session's signal (see {@link DetoxConnectOptions.signal});
...
/**
 * The typed view of every device type this client can allocate (spec 015):
 * Detox 20's legacy names with v20's query shapes, and any
 * driver package's own entry. A driver ships a declaration that merges into
 * this interface — the way a matcher package augments jest's `expect`:
 *
 *   declare module 'detox/client' {
 *     interface AllocationMap {
 *       '@acme/detox-driver-foo': { query: { region?: string }; info: { instanceId: string } };
 *     }
 *   }
 *
 * The wire carries `type`, the query and the descriptor verbatim: the server
 * resolves `type` to a driver, the driver reads its own query and describes
 * its own device. A path-shaped type is refused server-side, never here.
 */
export interface AllocationMap {
  'ios.simulator': { query: DeviceQuery; info: { udid: string } };
  'android.emulator': { query: { avdName?: string }; info: { adbName: string } };
  'android.attached': { query: { adbName?: string }; info: { adbName: string } };
  'android.genycloud': { query: { recipeUUID?: string; recipeName?: string }; info: { adbName: string } };
}

/** A device type: a key of {@link AllocationMap} — a legacy name, or a driver package that augmented the map. */
export type DeviceType = keyof AllocationMap;

/** The device query a given {@link DeviceType} takes — the driver's own vocabulary. */
export type DeviceQueryOf<T extends DeviceType> = AllocationMap[T]['query'];
 ...
 * and the caller-owned ready timeout. All optional — the frozen `{ signal }` calls
...
export interface LaunchAppOptions extends DetoxCallOptions {
  ...
  /**
   * How long the server waits for the whole launch (accepting the verb →
   * the app's own `ready`), in ms — a whole number up to 2^31-1. Absent →
   * the server default (120 s). `0` is legal and means no server-side
   * timeout — the caller's `signal` is the only exit, and that is a real
   * responsibility: an unbounded launch you never cancel is a pending
   * operation on the device, and `device.release()` waits pending
   * operations out (the reclaim barrier), so cancel the launch before
   * releasing.
   */
...
export interface DetoxApp extends AsyncDisposable {
  ...
  /**
   * Real OS pid of the launched process — externally verifiable. `undefined`
   * on an attached handle (spec 015): the frozen dialect carries no process
   * identity, so a pid exists only where a spawn returned one.
   */
   ...
   * Signal-only: no ready-timeout parameter.
   */
...
/**
 * The handle a spawn hands back ({@link DetoxDevice.launchApp},
 * {@link DeviceApps.launch}): a {@link DetoxApp} whose `pid` is always a real
 * number, since a launch returned one. Only attach/activate/connected handles
 * carry the widened `pid: number | undefined` (spec 015).
 */
export interface DetoxLaunchedApp extends DetoxApp {
  readonly pid: number;
}
...
/**
 * The `DeviceInfo` for a given {@link DeviceType}: the allocation's own fields
 * plus the driver's descriptor as its {@link AllocationMap} entry declares it
 * (`udid` on iOS, `adbName` on Android, whatever a driver package says).
 */
export type DeviceInfoOf<T extends DeviceType> = T extends DeviceType
  ? DeviceInfoBase & { readonly type: T } & Readonly<AllocationMap[T]['info']>
  : never;

/**
 * Static description of an allocated device — a discriminated union on `type`
 * over every {@link AllocationMap} entry, so `info.udid` / `info.adbName`
 * narrow instead of being optional everywhere.
 */
export type DeviceInfo = DeviceInfoOf<DeviceType>;

/**
 * The device's app collection (spec 015): a noun with a few verbs, plus the
 * device listener's address. `device.launchApp` stays as the frozen alias
 * for `apps.launch`.
 */
export interface DeviceApps {
  /**
   * The device's own app-facing listener address — what a manual launch must
   * dial (`-detoxServer` on iOS). A loopback address on the machine that owns
   * the device; informative through a relay, meaningful at that machine.
   */
  readonly serverUrl: string;
  /**
   * Launches the app and resolves with its handle once it reports ready —
   * always a fresh instance (a launch over a running one terminates it first;
   * the previous handle stays dead). The very same operation as
   * {@link DetoxDevice.launchApp}: its `id`/`name` are what `detox.on`
   * subscribers correlate the launch's progress with.
   */
  launch(bundleId: string, options?: LaunchAppOptions): DetoxOperation<DetoxLaunchedApp, 'launchApp'>;
  /**
   * Foreground the app if a session is connected and ready under it (a resume,
   * spec 006 — same process), else launch it with no launch options. Named
   * after `XCUIApplication.activate()`; it is Detox 20's default `launchApp()`
   * minus the options — signal-only, since an option that applies on one path
   * and is dropped on the other is a coin flip. Want args on a cold start?
   * That is {@link DeviceApps.launch}.
   */
  activate(bundleId: string, options?: AppCallOptions): Promise<DetoxApp>;
  /**
   * Resolves when an app is connected and ready under `sessionId` — at once if
   * one already is, otherwise when one arrives. Never spawns: it attaches to
   * an app launched outside Detox (Xcode, a finger) or by a previous test.
   * The session id is the bundle id by default (the frozen native's own
   * fallback). Signal-only; an abort rejects `DETOX_ABORTED`.
   */
  attach(sessionId: string, options?: AppCallOptions): Promise<DetoxApp>;
  /**
   * The device's connected, ready sessions as handles — remote state, one
   * wire call. A logged-in session not yet ready is not listed.
   */
  connected(options?: AppCallOptions): Promise<readonly DetoxApp[]>;
}
...
export interface DetoxDevice<Info extends DeviceInfo = DeviceInfo> extends AsyncDisposable {
   ...
   * language/locale, at-launch payloads as values, `readyTimeoutMs`.
   */
...
export interface Detox extends AsyncDisposable {
  ...
  /**
   * The id of this run's log on the server (spec 012): the key of
   * `GET /v1/runs/<id>/log` on the same host, port and token. Minted by the
   * server and announced on its first frame; `undefined` on an endpoint
   * that records no log (an older server, or a relay predating spec 008's
   * own log), where {@link DetoxLog} refuses typed.
   */
  readonly runId: string | undefined;

  /**
   * The run's log, written on the server (spec 012). `detox.log('…')` is one
   * line; `detox.log.begin(…)` opens a step. Every RPC this session makes
   * inside a step's `run` (or `detox.step(name, fn)`) is recorded under that
   * step — explicitly, so overlapping bodies never share one (spec 013);
   * outside any, under the most recently begun still-open step at the
   * moment the request frame is sent. Lines and steps nest the same way.
   * Every call throws `DETOX_NOT_IMPLEMENTED` — "this endpoint does not
   * record a log" — when the server announced none; never a silent drop.
   */
  readonly log: DetoxLog;

  /**
   * A user-named section of the run: `detox.step('login')` opens one and
   * hands back its handle (end it yourself, in any order); `detox.step('login',
   * fn)` runs `fn` inside it and ends it for you — `passed` when `fn`
   * returns, `failed` (carrying the error) when it throws, `aborted` when it
   * threw an `AbortError`. Sugar over {@link DetoxLog.begin} with `kind: 'step'`.
   */
  step(name: string): DetoxLogHandle;
  step<T>(name: string, fn: () => T | Promise<T>): Promise<T>;
}

/** The closed set of step kinds (spec 012); `attach` is reserved for a later revision. */
export type DetoxLogKind = 'file' | 'describe' | 'test' | 'hook' | 'step';

export type DetoxLogStatus = 'passed' | 'failed' | 'skipped' | 'aborted';

export type DetoxLogLevel = 'error' | 'warn' | 'info' | 'debug';

/** JSON scalars and arrays of scalars only — a nested object makes the server replace the whole `attrs` with `{ attrsRejected: true }`. */
export type DetoxLogAttrScalar = string | number | boolean | null;
export type DetoxLogAttrs = Record<string, DetoxLogAttrScalar | DetoxLogAttrScalar[]>;

/** Structured data on one log line, carried verbatim as the line's `fields`. */
export type DetoxLogFields = Record<string, unknown>;

export interface DetoxLogBeginOptions {
  kind: DetoxLogKind;
  name: string;
  attrs?: DetoxLogAttrs;
  /**
   * The step this one nests under (its handle's `id`). Absent, the step
   * nests under whatever step the caller is running inside (see
   * {@link DetoxLogHandle.run}), else under the run itself. Explicit
   * beats ambient — a runner integration parents describe and hook steps
   * this way, since they begin outside any test body.
   */
  parent?: string;
}

export interface DetoxLogEnd {
  status: DetoxLogStatus;
  /** Carried as `{ name, message }` — never the stack, never parsed. */
  error?: { name: string; message: string };
}

/** A step handle: its client-minted id (`step:<id>` in the log), its one `end`, and its context. */
export interface DetoxLogHandle {
  readonly id: string;
  end(outcome: DetoxLogEnd): void;
  /** Whether `end` was called on this handle (the server, not the client, refuses a second end). */
  readonly ended: boolean;
  /**
   * Runs `fn` inside this step: every Detox call it makes — however deep,
   * across every `await` — is recorded as this step's child, and every
   * step it begins nests under this one. Concurrency-safe: two bodies
   * running at once each see their own step (spec 013's `test.concurrent`
   * guarantee). Returns whatever `fn` returns; ending the step is still
   * the caller's (`detox.step(name, fn)` does both).
   */
  run<T>(fn: () => T): T;
}

/** One line at a fixed level: a message, or fields plus a message. */
export interface DetoxLogWriter {
  (msg: string): void;
  (fields: DetoxLogFields, msg: string): void;
}

/**
 * The log namespace: callable (an `info` line), one writer per level, and
 * `begin` for a typed step. Lines land under the most recently begun
 * still-open step, else under the run's own node; steps and requests made
 * inside a step's {@link DetoxLogHandle.run} nest under that step.
 */
export interface DetoxLog extends DetoxLogWriter {
  readonly error: DetoxLogWriter;
  readonly warn: DetoxLogWriter;
  readonly info: DetoxLogWriter;
  readonly debug: DetoxLogWriter;
  begin(options: DetoxLogBeginOptions): DetoxLogHandle;
}
 ...
 */
export { statelessBy as by } from './client/app';

/**
 * Rejected by entry points of this module that a future spec still has to
 * implement (`connect` itself graduated with spec 001).
 */
 ...
 * declared return type is a promise and `connect(…).catch(…)` must reach the
 * handler.
 */
export function connect(options: DetoxConnectOptions): Promise<Detox> {
  return connectSession(options);
}
