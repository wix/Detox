...
class IPCServer {
   ...
   * @param {() => Promise<DeviceCookie>} options.callbacks.onAllocateDevice
   * @param {(cookie: DeviceCookie) => Promise<void>} options.callbacks.onDeallocateDevice
   */
  ...
  async onAllocateDevice(_payload, socket = null) {
    try {
      ...
      this._ipc.server.emit(socket, 'allocateDeviceDone', { error });
    }
  }
  ...
  async onDeallocateDevice({ deviceCookie }, socket) {
    try {
      ...
      this._ipc.server.emit(socket, 'deallocateDeviceDone', { error });
    }
  }
