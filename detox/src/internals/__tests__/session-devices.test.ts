...
import type { DetoxOperationRef, DetoxProgressEvent } from '../../internals';
...
describe('DetoxSession — device lifecycle', () => {
  ...
  /**
   * @issue DTX-3008
   * The public dialect's device query says `model`; applesimutils calls the
   * same thing `type`. `id`/`os` pass through as-is.
   */
  it('allocates a device, mapping the wire query into the applesimutils dialect', async () => {
    ...
    expect(receivedRequest?.device).toEqual({ id: 'ABCD-1234', type: 'iPhone 17', os: '17.2' });
