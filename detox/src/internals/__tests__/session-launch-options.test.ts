...
import type { DetoxApp, DetoxDevice, DetoxOperationRef } from '../../internals';
...
describe('launchApp options travel verbatim, and only when present', () => {
  ...
  it('deadlineMs: 0 reaches the wire as a value — presence is !== undefined, not truthiness', async () => {
    const { device, received, dispose } = await withApp();
    await device.launchApp('com.example.app', { deadlineMs: 0 });
    expect(received.at(-1)?.params.deadlineMs).toBe(0);
    await dispose();
  });
});
