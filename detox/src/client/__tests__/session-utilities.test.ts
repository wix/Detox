...
import type { DetoxDevice, DetoxOperationRef, DetoxProgressEvent } from '../../client';
...
async function withDevice(options: { signal?: AbortSignal } = {}): Promise<{
  ...
  const sessionPromise = connect({ server: 'ws://fake-host/detox', signal: options.signal });
...
describe('DetoxDevice — the utilities toolbelt', () => {
  ...
  /**
   * A call inherits the session's signal (`connect({ signal })`), so tearing the
