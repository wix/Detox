...
import type { DetoxApp, DetoxDevice } from '../../client';
...
describe('DetoxDevice — device.apps (spec 015)', () => {
  it('exposes serverUrl and the launch/activate/attach/connected verbs', async () => {
    const { device, received, server, dispose } = await withDevice();
    try {
      expect(device.apps.serverUrl).toBe('ws://127.0.0.1:5599');

      // `launch` is the same operation as `device.launchApp` (the frozen alias).
      const launched = await device.apps.launch('com.example.app');
      expect(launched.pid).toBe(4242);
      expect(launched.bundleId).toBe('com.example.app');

      server.onRequest<Record<string, unknown>, { appHandleId: string; bundleId: string; pid?: number }>(
        'attachApp',
        async (params) => {
          received.push({ method: 'attachApp', params });
          return { appHandleId: 'att-1', bundleId: 'com.example.app', pid: 4242 };
        },
      );
      const attached = await device.apps.attach('com.example.app');
      expect(attached.bundleId).toBe('com.example.app');
      expect(received.at(-1)?.params).toMatchObject({ allocationId: 'alloc-1', sessionId: 'com.example.app' });

      server.onRequest<Record<string, unknown>, { appHandleId: string; bundleId: string; pid?: number }>(
        'activateApp',
        async (params) => {
          received.push({ method: 'activateApp', params });
          return { appHandleId: 'act-1', bundleId: 'com.example.app' };
        },
      );
      const activated = await device.apps.activate('com.example.app');
      expect(activated.bundleId).toBe('com.example.app');
      // Attached/activated handles carry no pid unless a spawn returned one.
      expect(activated.pid).toBeUndefined();
      expect(received.at(-1)?.params).toMatchObject({ allocationId: 'alloc-1', appId: 'com.example.app' });

      server.onRequest<Record<string, unknown>, { apps: Array<{ appHandleId: string; bundleId: string; pid?: number }> }>(
        'connectedApps',
        async (params) => {
          received.push({ method: 'connectedApps', params });
          return { apps: [{ appHandleId: 'c-1', bundleId: 'com.example.app', pid: 4242 }] };
        },
      );
      const connected = await device.apps.connected();
      expect(connected).toHaveLength(1);
      expect(connected[0].bundleId).toBe('com.example.app');
    } finally {
      await dispose();
    }
  });
