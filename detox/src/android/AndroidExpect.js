...
class AndroidExpect {
  ...
  element(matcher) {
    if (matcher instanceof NativeMatcher) {
      return new NativeElement(this._device, this._emitter, matcher);
    }
  ...
  expect(element) {
    if (element instanceof WebElement) return new WebExpectElement(this._device, element);
    if (element instanceof NativeElement) return new NativeExpectElement(this._device, element);
    throw new DetoxRuntimeError(`expect() argument is invalid, expected a native or web matcher, but got ${typeof element}`);
  }

  waitFor(element) {
    if (element instanceof NativeElement) return new NativeWaitForElement(this._device, element);
