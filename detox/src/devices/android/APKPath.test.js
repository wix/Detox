describe("APKPath", () => {
	...
	it(`path for a gradle build flavor`, async () => {
		const inputApkPath =
			"~/somePath/build/outputs/apk/development/debug/app-development-debug.apk";
		const expectedTestApkPath =
			"~/somePath/build/outputs/apk/androidTest/development/debug/app-development-debug-androidTest.apk";
		expect(APKPath.getTestApkPath(inputApkPath)).toEqual(expectedTestApkPath);
	});
});
