...
/**
 * @typedef CloudAndroidDriverProps
...
class CloudAndroidDriver extends DeviceDriverBase {
  ...
  async _handleLaunchApp({ manually, bundleId, launchArgs }) {
    ...
    await this.emitter.emit('beforeLaunchApp', { bundleId, launchArgs });
    ...
    // } else {
    const response = await this._launchApp(adbName, bundleId, launchArgs);
    // }
    // _.get(deviceConfig, 'device.name')
  ...
  // Cannot find initialization https://github.com/pb2323/cloud_detox_support/blob/2382f0b3b0d86c0572dcfa915b412c12c8ace4f0/detox/src/android/core/WebElement.js#L39
  async typeText(text) {
  ...
  //   // const { adbName } = this;
  //   await this.emitter.emit('beforeTerminateApp', { bundleId });
  ...
  //   } catch (e) {
  //     // Show generic message sent by hub
  ...
  //   // This should be a http request
  //   // "/Users/puneetbajaj/Library/Android/sdk/platform-tools/adb" -s RFCR90M7DPD shell "ps | grep \"com\.detox\.rn\.example$\""
  //   const pid = await this.invocationManager.executeCloudAdb({
  //     'method': 'pidOf',
  //     'target': {
  //       'type': 'ps',
  //       'params': {
