...
class Fbsimctl {
  ...
  async list(device) {
    ...
    if (!simId) {
      throw new Error(
        "Can't find a simulator to match with '" +
          device +
          "', run 'fbsimctl list' to list your supported devices.\n" +
  ...
  async boot(udid) {
    ...
    await retry({ retries: 10, interval: 1000 }, async () => {
      const initialStateCmdResult = await this._execFbsimctlCommand(
        { args: `${udid} list` },
        undefined,
        1
      );
      initialState =
        _.get(initialStateCmdResult, "stdout", "") === ""
          ? undefined
          : _.get(
              JSON.parse(_.get(initialStateCmdResult, "stdout")),
              "subject.state"
            );
    ...
    return await this._execFbsimctlCommand(
      { args: `--state booted ${udid} list` },
      {
  ...
  async sendToHome(udid) {
    const result = await exec.execWithRetriesAndLogs(
      `/usr/bin/xcrun simctl launch ${udid} com.apple.springboard`
    );
    return parseInt(result.stdout.trim().split(":")[1]);
  }
  ...
  async resetContentAndSettings(udid) {
    ...
    const result = await exec.execWithRetriesAndLogs(
      `/usr/bin/xcrun simctl erase ${udid}`
    );
