...
class AppleSimUtils {
  async setPermissions(udid, bundleId, permissionsObj) {
    ...
    await this._execAppleSimUtils(options, 1);
  }
  ...
  async list(query, listOptions = {}) {
    ...
    const response = await this._execAppleSimUtils(options,  1);
  ...
  async setLocation(udid, lat, lon) {
    const result = await exec.execWithRetriesAndLogs(`which fbsimctl`, undefined, 1);
    if (_.get(result, 'stdout')) {
      await exec.execWithRetriesAndLogs(`fbsimctl ${udid} set_location ${lat} ${lon}`, undefined, 1);
    } else {
  ...
  async _execAppleSimUtils(options, retries, interval) {
    const bin = `applesimutils`;
    return await exec.execWithRetriesAndLogs(bin, options, retries, interval);
  }
  ...
  async _execSimctl({ cmd, statusLogs = {}, retries = 1, silent = false }) {
    ...
    return await exec.execWithRetriesAndLogs(`/usr/bin/xcrun simctl ${cmd}`, options, retries);
  }
  ...
  async _launchMagically(frameworkPath, udid, bundleId, launchArgs, languageAndLocale) {
    ...
    const result = await exec.execWithRetriesAndLogs(launchBin, options, 1);
    return result;
  }
