describe('ADB', () => {
  ...
  describe('devices', () => {
    ...
    it(`should invoke ADB`, async () => {
      await adb.devices();
      expect(exec).toHaveBeenCalledWith(`"${adbBinPath}"  devices`, { verbosity: 'high' }, 1);
  ...
  describe('animation disabling', () => {
    it('should disable animator (e.g. ObjectAnimator) animations', async () => {
      ...
      expect(exec).toHaveBeenCalledWith(`"${adbBinPath}"  shell "settings put global animator_duration_scale 0"`, undefined, 1);
    });

    it('should disable window animations', async () => {
      await adb.disableAndroidAnimations();
      expect(exec).toHaveBeenCalledWith(`"${adbBinPath}"  shell "settings put global window_animation_scale 0"`, undefined, 1);
    });

    it('should disable transition (e.g. activity launch) animations', async () => {
      await adb.disableAndroidAnimations();
      expect(exec).toHaveBeenCalledWith(`"${adbBinPath}"  shell "settings put global transition_animation_scale 0"`, undefined, 1);
    });
  });
});
